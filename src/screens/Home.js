...
export default function Home() {
  ...
  <div key={data.id} className='fs-3 m-3 col-12'>{data.CategoryName}</div>
  <hr id="hr-danger" className="col-12 bg-white" style={{ height: "6px"}} />
  {
    foodItems !== [] ? 
    foodItems.filter(
      (items) => (items.CategoryName === data.CategoryName) && (items.name.toLowerCase().includes(search.toLowerCase()))).map(filterItems => 
        {
          return (
            <div key={filterItems.id} className='col-12 col-md-6 col-lg-3 d-flex justify-content-center'>
